...
void Bullets::paintGL() {
  ...
  for (const auto &bullet : m_bullets) {
    ...
    abcg::glUniform4fv(m_colorLoc, 1, &bullet.m_color.r);
...
void Bullets::update(Ship &ship, const GameData &gameData, float deltaTime) {
  ...
  if (gameData.m_input[static_cast<size_t>(Input::Fire)] &&
    ...
    // At least 150 ms must have passed since the last bullets
    if (ship.m_bulletCoolDownTimer.elapsed() > 150.0 / 1000.0) {
